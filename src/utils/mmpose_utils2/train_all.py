...
def create_normal_train_config(
    ...
    base_pipeline_with_heatmap = [
        dict(type='LoadImage'),
        dict(type='GetBBoxCenterScale'),
        dict(type='RandomBBoxTransform',shift_prob=0.0, scale_prob=0.0, rotate_prob=0.0),
        dict(input_size=input_img_size, type='TopdownAffine'),
        dict(
            encoder=dict(
                heatmap_size = tuple(int(x/4) for x in input_img_size),
    ...
    cfg.train_dataloader.dataset.pipeline = base_pipeline_with_heatmap
    cfg.train_pipeline                    = base_pipeline_with_heatmap
    ...
    cfg.val_dataloader.dataset.pipeline = base_pipeline
    cfg.val_pipeline                    = base_pipeline
    ...
    cfg.test_dataloader.dataset.pipeline = base_pipeline
    cfg.test_pipeline                    = base_pipeline
