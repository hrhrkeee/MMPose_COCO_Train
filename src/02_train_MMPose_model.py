...
def parse_args():
    ...
    parser.add_argument(              '--model_root',             type=str, required=False, default="./__model__/04_mmpose/")
    parser.add_argument(              '--val_interval',           type=int, required=False, default=1)
...
def train_mmpose_model(args):
    ...
    dt_now = datetime.datetime.now().strftime('%Y%m%d-%H%M%S') + f"{random_num:02d}"
    pose_model_name = "-".join(Path(args.base_pose_model).stem.split('_')[:2])
    
    subdir_name = f"{dt_now}__{pose_model_name}_{Path(args.train_coco_path).parent.parent.stem}"
    ...
    config_path = util.create_original_mmpose_config(
                    ...
                    pose_model_root=args.model_root,
                    seed = args.seed,
                )
